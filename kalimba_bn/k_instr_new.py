...
class KalimbaIndexedMemAccess:
    ...
    def llil(self, il: LowLevelILFunction, addr: int, length: int):
        if isinstance(self.mod, int):
            mod_expr = il.const(4, self.mod)
        else:
            mod_expr = il.reg(4, self.mod.name)
        if self.write:
            il.append(il.store(4, il.reg(4, self.idx.name), il.reg(4, self.reg.name)))
        else:
            il.append(il.set_reg(4, self.reg.name, il.load(4, il.reg(4, self.idx.name))))
        il.append(il.set_reg(4, self.idx.name, il.add(4, il.reg(4, self.idx.name), mod_expr)))
        return
...
class KalimbaUnOp:
    ...
    def llil(self, il: LowLevelILFunction, addr: int, length: int):
        if self.op == KalimbaOp.DIV:
            il.append(il.set_reg(4, self.c.name, il.reg(4, self.a.name)))
        if self.mem1:
            self.mem1.llil(il, addr, length)
        if self.mem2:
            self.mem2.llil(il, addr, length)
        return
...
class KalimbaBinOp:
    ...
    def llil(self, il: LowLevelILFunction, addr: int, length: int):
        if self.cond != KalimbaCond.Always:
            t0 = LowLevelILLabel()
            f0 = LowLevelILLabel()
            il.append(il.if_expr(cond_to_flags(self.cond, il), t0, f0))
            il.mark_label(t0)
        value_expr = None
        if self.op == KalimbaOp.ADD:
            ...
            il.mark_label(t)
            shift_value_expr = il.logical_shift_right(4, src_op_to_expr(il, self.a), il.neg_expr(4, src_op_to_expr(il, self.b)), flags='znvc')
            dst_op_to_expr(il, self.c, shift_value_expr)
            il.append(il.goto(exit_label))
            il.mark_label(f)
            shift_value_expr = il.shift_left(4, src_op_to_expr(il, self.a), src_op_to_expr(il, self.b), flags='znvc')
            dst_op_to_expr(il, self.c, shift_value_expr)
            ...
            il.mark_label(t)
            shift_value_expr = il.arith_shift_right(4, src_op_to_expr(il, self.a), il.neg_expr(4, src_op_to_expr(il, self.b)), flags='znvc')
            dst_op_to_expr(il, self.c, shift_value_expr)
            il.append(il.goto(exit_label))
            il.mark_label(f)
            shift_value_expr = il.shift_left(4, src_op_to_expr(il, self.a), src_op_to_expr(il, self.b), flags='znvc')
            ...
            pass
        elif self.op == KalimbaOp.DIV:
            il.append(il.set_reg(4, KalimbaBank3Reg.DivResult.name, il.div_signed(4, src_op_to_expr(il, self.a), src_op_to_expr(il, self.b))))
            il.append(il.set_reg(4, KalimbaBank3Reg.DivRemainder.name, il.mod_signed(4, src_op_to_expr(il, self.a), src_op_to_expr(il, self.b))))
        if value_expr:
...
def kalimba_maxim_decode_fmaddsub_creg(instruction, op, prefix):
    (mem1, mem2, rega, regc) = kalimba_maxim_decode_creg_c(instruction)
    sign = KalimbaSignSelect(get_bits(instruction, 26, 2))
...
class KalimbaStackOp:
    ...
    def __str__(self):
        ...
        if len(self.reg) == 1:
            reg = self.reg[0].name
        elif len(self.reg) > 1 or len(self.reg) == 0:
...
maxim_ops_lut: List[Union[Tuple[int, int, KalimbaOp, Callable[[int, KalimbaOp, Optional[KalimbaPrefix]], KalimbaInstruction]], Tuple[int, int, KalimbaOp]]] = [
    ...
    (0b111100_11_00000000_00000000_00000000, 0b101_100_10_00000000_00000000_00000000, KalimbaOp.FMSUB, kalimba_maxim_decode_fmaddsub_creg),
    ...
    (0b111100_11_00000000_00000000_00000000, 0b101_100_11_00000000_00000000_00000000, KalimbaOp.FMSUB, kalimba_maxim_decode_fmaddsub_const),
...
def test():
    ...
    (i, o, decode) = kalimba_maxim_lookup_decoder(int.from_bytes(bytes.fromhex('341200fd'), 'little'))
    prefix = decode(i, o, None)
    ...
    for (data, asm) in kalimba_maxim_instructions_test:
        try:
            ...
            asm = asm.replace(';', '').replace(' EQ ', ' Z ').replace(' NE ', ' NZ ')

            asm_str = f'{type(op).__name__}("{asm}")'
            op_str = f'{op}'

            # This is quite a special case, can't be bothered to fix
            if op_str != asm_str and op_str.replace(' (SS)', '') != asm_str: # pragma: no cover
                (instruction, opcode, decode) = kalimba_maxim_lookup_decoder(int.from_bytes(data[-4:], 'little'))
